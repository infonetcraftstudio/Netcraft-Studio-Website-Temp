  ...
  <div style="background:#f4f7fb;padding:40px 16px;font-family:Arial,sans-serif;color:#162033">
    <div style="max-width:620px;margin:0 auto;background:#ffffff;border:1px solid #dbe3ee">
      <div style="background:#10233f;padding:28px 32px;color:#ffffff">
        <div style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#8ec5ff">
          NetCraft Studio
        </div>

        <h1 style="font-size:28px;line-height:1.2;margin:16px 0 0;font-weight:600">
          ${title}
        </h1>
      </div>

      <div style="padding:30px 32px">
        <div style="font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:#2563eb;margin-bottom:18px">
          ${eyebrow}
        </div>
      ...
      <div style="border-top:1px solid #e6ebf2;padding:18px 32px;color:#718096;font-size:12px">
        NetCraft Studio · Coimbatore, Tamil Nadu
      </div>
    </div>
  </div>
`;
...
export default async function handler(request, response) {
  ...
  try {
    ...
    const details = `
      <p style="font-size:15px;line-height:1.7;margin:0 0 22px">
        A new ${
          isReview
            ? 'client review'
            : 'project enquiry'
        } was submitted through the website.
      </p>
      ...
      <p style="font-size:11px;letter-spacing:1px;text-transform:uppercase;color:#718096;margin:0 0 8px">
        ${
          isReview
            ? 'Client Review'
      ...
      <p style="font-size:15px;line-height:1.8;margin:0;white-space:pre-wrap">
        ${message}
      </p>
    `;
    ...
    const clientMessage = `
      ...
      <p style="font-size:15px;line-height:1.8;margin:0 0 16px">
        Thank you for sharing your ${
          isReview
            ? `feedback on ${escapeHtml(
                inquiry.reviewFor || service
              )}`
            : 'project requirements'
        } with NetCraft Studio.
      </p>

      <p style="font-size:15px;line-height:1.8;margin:0 0 24px">
        Your message is safely with our team.
        A senior studio partner will review it
      ...
      <div style="
        ...
        line-height:1.7
      ">
        We appreciate the opportunity to build
        thoughtful digital work with you.
      </div>
    `;
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject:
        'Thank you for contacting NetCraft Studio',
      html: emailLayout(
        'Message received',
